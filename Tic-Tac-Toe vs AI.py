...
def playerMove():
    run = True
    while run:
        move = input('Please select a position from 1-9 to place your "X". ')
        try:
            move = int(move)
            if move > 0 and move < 10:
                ...
                print('Your move was out of bounds!')
        except:
    ...
    while not isBoardFull(board):
        ...
        if not (isWinner(board, 'X')):
            ...
            if move == 0:
                ...
                insertLetter('O', move)
                print(f'Computer inserted an O in position {move}. ')
                printBoard(board)
        else:
